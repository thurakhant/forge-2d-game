...
class MyPhysicsGame extends Forge2DGame {
  ...
  Future<void> addGround() {
    return world.addAll([
      for (var x = camera.visibleWorldRect.left; x < camera.visibleWorldRect.right + groundSize; x += groundSize)
        Ground(
    ...
    for (var i = 0; i < 5; i++) {
      ...
      await world.add(
        Brick(
          ...
          position: Vector2(camera.visibleWorldRect.right / 3 + (_random.nextDouble() * 5 - 2.5), 0),
  ...
  Future<void> addEnemies() async {
    ...
    for (var i = 0; i < 3; i++) {
      await world.add(
        Enemy(
          Vector2(camera.visibleWorldRect.right / 3 + (_random.nextDouble() * 7 - 3.5), (_random.nextDouble() * 3)),
